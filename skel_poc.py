...
import numpy as np
from anatomical_atlas import load_atlas_registered, load_atlas_transform_plan, apply_atlas_transform, ensure_bodyparts3d_atlas, bodyparts3d_atlas_status
import pandas as pd
...
def _fit_skel_sequence(torch, model, motion, seed_fit, start_index=0, iterations=16, progress_cb=None, empirical_profile=None, acquisition_mode="monocular_depth_safe"):
    ...
    result={'version':'110.3.20.17-segment-gentle-anatomical-layers','retargeting_mode':'fullbody_segment_frame_reference_skel_skeleton_layer','scale':float(scale.cpu()),
...
def _skel_volumetric_anatomy_2010(joints, joint_names):
    """V110.3.20.17 · Generates volumetric anatomically-shaped bone and muscle meshes.
...
def _plot_skel_mesh_walker(mesh_seq):
    """V110.3.20.17 · visor de atlas anatómico REAL sobre SKEL. Sin fallback geométrico."""
    try:
        ...
        if not _atlas_status.get("ready"):
            st.warning("V110.3.20.17 · Atlas BodyParts3D real todavía no está instalado en esta instancia.")
        ...
        # LAZY: no cargar anatomía hasta que el usuario la pida.
        _layer_choice=st.radio(
            "Capa anatómica a preparar",
            ["Piel","Huesos reales","Músculos reales","Todas"],
            horizontal=True,
            key="v2017_layer_choice"
        )
        _groups=()
        if _layer_choice=="Huesos reales": _groups=("bones",)
        elif _layer_choice=="Músculos reales": _groups=("muscles",)
        elif _layer_choice=="Todas": _groups=("bones","muscles")

        atlas={"bones":[],"muscles":[],"missing":[]}
        if _groups and _atlas_status.get("ready"):
            with st.spinner("Preparando transformaciones anatómicas ligeras…"):
                atlas=_v2017_atlas_plan_for_mesh(mesh_seq,groups=_groups)
            st.success(
                f"✅ Capa preparada · {len(atlas.get('bones',[]))} huesos · "
                f"{len(atlas.get('muscles',[]))} músculos · sin duplicar vértices ×75"
            )
            st.caption("V110.3.20.17 conserva una única malla por estructura + matrices/escala/traslación por frame.")
        elif _groups and not _atlas_status.get("ready"):
            st.info("Prepara primero el atlas BodyParts3D para habilitar Huesos/Músculos.")
        ...
        # Payload LIGHT: skin sequence + anatomical base meshes once + transforms.
        P={"V":np.round(Vviz,5).tolist(),"F":F[:,:3].tolist(),"ids":ids,"ranges":ranges,
           "layer":_layer_choice,
           "bones":[{"id":x["id"],"V0":np.round(x["V0"],5).tolist(),"F":x["F"].tolist(),
                     "R":np.round(x["R"],5).tolist(),"S":np.round(x["S"],6).tolist(),"T":np.round(x["T"],5).tolist()} for x in bone_parts],
           "muscles":[{"id":x["id"],"V0":np.round(x["V0"],5).tolist(),"F":x["F"].tolist(),
                       "R":np.round(x["R"],5).tolist(),"S":np.round(x["S"],6).tolist(),"T":np.round(x["T"],5).tolist()} for x in muscle_parts]}
        ...
        Plotly.newPlot(gd,data,{{title:'V110.3.20.17 · Anatomical Atlas Mesh',margin:{{l:0,r:0,t:45,b:0}},scene:{{aspectmode:'data',xaxis:{{range:P.ranges[0]}},yaxis:{{range:P.ranges[1]}},zaxis:{{range:P.ranges[2]}}}}}},{{responsive:true,displaylogo:false}});
        ...
        if not _atlas_status.get("ready"):
            ...
            st.caption("V110.3.20.17 · Render anatómico bajo demanda: sólo se carga la capa seleccionada para reducir memoria.")
    except Exception as exc:
...
def _load_mesh_sequence_npz_bytes_2012(raw):
    """V110.3.20.17 · Reabre un NPZ SKEL ya calculado sin volver a generar los 75 frames."""
    try:
...
def render_skel_poc_panel(motion, acquisition_mode="monocular_depth_safe", acquisition_label=None):
    ...
    st.caption("V110.3.20.17 · Carga el NPZ descargado de este registro para abrir directamente sus 75 frames SKEL. No repite fitting, retargeting ni generación de los 75 frames.")
